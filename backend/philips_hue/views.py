...
def update_light(request,pk):
    ...
    try:
        ...
        logger.warning("I am calling the update_lights function in the view. lights_group is {}".format(light))
        logger.warning('the light {} status is {}.'.format(light.light_id,light.on))
        if light.on.on:
    ...
    if room and room.services:
        for service in room.services:
            if service['rtype'] == 'grouped_light':
                try:
                    grouped_lights = fetch_resource_endpoint_detail_task(endpoint=service['rtype'],identifier=service['rid'])
                    
                    for light in grouped_lights:
                        light_info = {
                            'rtype': light.get('type', 'Unknown'),
                            'rid': light.get('id', 'Unknown'),
                            'on_status': 'true' if light.get('on', {}).get('on', False) else 'false',  # Convert to JavaScript boolean,
                    ...
                    logger.error(f"Error getting lights for device {service['rid']}: {e}")
